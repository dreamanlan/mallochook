...
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG, "inject", __VA_ARGS__)
...
{
	int ret = -1;
	void *mmap_addr, *dlopen_addr, *dlsym_addr, *dlclose_addr, *dlerror_addr, *puts_addr;
	...
	extern uint32_t _dlopen_addr_s, _dlopen_param1_s, _dlopen_param2_s, _dlsym_addr_s, \
			_dlsym_param2_s, _dlclose_addr_s, _dlerror_addr_s, _puts_addr_s, \
	...
	_dlerror_addr_s = (uint32_t)dlerror_addr;
	_puts_addr_s = (uint32_t)puts_addr;
...
int main(int argc, char** argv) {
	...
	for(int i=1;i<argc;++i){
		...
		if(0==strcmp(p,"-pid") && i+1<argc){
			...
			puts("\tinject process_id");
			puts("\tinject -pid process_id");
			puts("\tinject -name com.xxx.xxx");
			puts("\tinject -pid process_id -so xxx.so");
	...
	int ret = inject_remote_process(target_pid, path, func, argstr, strlen(argstr)+1);
