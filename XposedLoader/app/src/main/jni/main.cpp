...
static int hook()
{
	...
	if (registerInlineHook((uint32_t)malloc, (uint32_t)my_malloc, (uint32_t **)&old_malloc_ptr) != ELE7EN_OK) {
		__android_log_print(ANDROID_LOG_INFO, "inject", "register malloc hook failed!");
		r = -1;
	}
	if (inlineHook((uint32_t)malloc) != ELE7EN_OK) {
		__android_log_print(ANDROID_LOG_INFO, "inject", "malloc hook failed!");
		r = -1;
	}
	if (registerInlineHook((uint32_t)free, (uint32_t)my_free, (uint32_t **)&old_free_ptr) != ELE7EN_OK) {
		__android_log_print(ANDROID_LOG_INFO, "inject", "register free hook failed!");
		r = -1;
	}
	if (inlineHook((uint32_t)free) != ELE7EN_OK) {
		__android_log_print(ANDROID_LOG_INFO, "inject", "free hook failed!");
		r = -1;
	}
...
extern "C" {
	...
	{
		g_CanLog = 0;
		__android_log_print(ANDROID_LOG_INFO, "inject", "UninstallHook\n");
		printf("UninstallHook\n");
		unHook();
		__android_log_print(ANDROID_LOG_INFO, "inject", "UninstallHook finish.\n");
